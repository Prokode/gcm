...
router.post('/create', venteReqValidators.validate('create'), function (req, res, next) {
    ...
    try {
        ...
        const countDown = new Stopwatch(time_init, options);

        //Socket connection handler
        io.on('connection', function (socket) {
            .onTime(function(){
                //console.log('time');
                 gameTableArray.forEach(function(value,index,array){
                         if(value.poste == data.poste){
                           value.time = value1.timeChange();
                        }
                 });
        if(state == 'low') {
                  tele1.on();
           ...
           socket.emit('timeChange',{'time': msToTime(value1.showValueTime()),'poste':data.poste });
         
         //console.log(myPoste.showValueTime());
        
        //if( myPoste.timeChange().getTime() == myPoste.timeToDo ){myPoste.timeIsEnd = true; myPoste.stopTime();}
