        ...
        return User.find({username:  username}, function(err, users) {
            ...
            try {
                if (!users.length) {
                    ...
                    const user = users[0];
                    console.log(phash.verify(password, "sha1$7058a8a5$1$89cb262dc0ce3e085a2d26879c860beead22babe"));
...
passport.use(new JWTStrategy({
        ...
        return Auth.findOne({toke: token}, function(err, auth) {
            if (err) {
                error.status = 500;
                cb(error);
            }
            try {
                ...
                if (!auth.length) {
                    ...
                    const authPayLoad = auth[0]; 
                    if (authPayLoad.user_id === jwtPayload.id && authPayLoad.delated_at === null) {
