...
router.get('/create', consoleReqValidators.validate('create'), function (req, res, next) {
    let error = new Error();
    try {
        ...
        const activation = new Console(body.token);

        db.insertTableContent('consoles', globals.DB_PATH, activation, (succ, msg) => {
            if (!succ) {
            error.status = 500;
            next(error);
          } else {
            ...
            db.getAll('consoles', globals.DB_PATH, (succ, data) => {
                ...
                try { 
                    const dataFilt = data.filter(function(d) {
                        return base64.decode(d.name) === req.query.name;
                    });
