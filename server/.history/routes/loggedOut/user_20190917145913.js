...
const { validationResult } = require('express-validator/ch');
...
router.get('/username/check', userReqValidators.validate('checkUsername'), function (req, res, next) {
    ...
    try {
        ...
        if (!checkDB) {
            ...
            db.getAll('users', globals.DB_PATH, (succ, data) => {
                if (succ) {
                    ...
                    const dataFilt = data.filter(function(d) {
                        return base64.decode(d.username) === req.query.username;
                    });
            ...
            // db.search('users', globals.DB_PATH, 'username', base64.encode(req.query.username), (succ, data) => {
            //     if (data.isEmpty()) {
            //         res.send({
            //             unused: false
            //         });
