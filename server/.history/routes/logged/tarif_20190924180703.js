    ...
    try {
        ...
        if (!checkDB) {
            ...
            db.getAll('consoles', globals.DB_PATH, (succ, dataConsole) => {
                ...
                var consoles = [];
                var tarifs = [];
                dataConsole.reverse().forEach(console => {
                    db.search('tarifs', globals.DB_PATH, 'console_id', console.id, (succ, dataTarifs) => {
                        if(succ) {
                            tarifs = dataTarifs
                        }
                    });
                    consoles.push(
                        {
                            console: {
                                name: base64.decode(console.name),
                                id: console.id
                            },
                            tarifs: tarifs
                        }
...
router.post('/create', tarifReqValidators.validate('create'), function (req, res, next) {
    ...
    try {
        ...
        const tarif = new Tarif(body.console_id, body.cost, time, req.user.userId);

        db.insertTableContent('tarifs', globals.DB_PATH, tarifs, (succ, msg) => {
            if (!succ) {
            error.status = 500;
            next(error);
          } else {
