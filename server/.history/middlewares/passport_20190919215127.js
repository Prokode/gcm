...
passport.use(new LocalStrategy({
    ...
    function (req, username, password, cb) {
        return db.getAll('users', globals.DB_PATH, (succ, data) => {
                var error = new Error();
                if (!succ) {
                    error.status = 500;
                    cb(error);
...
passport.use(new JWTStrategy({
        jwtFromRequest: ExtractJWT.fromAuthHead ('authorization'),
    ...
    function (req, jwtPayload, cb) {
        ...
        // db.getRows('auths', globals.DB_PATH, {
        //     token: token
        //   }, (succ, result) => {
        //     if (succ) {
        //         const auth = result; 
        //         if (auth.userId === jwtPayload.id) {
