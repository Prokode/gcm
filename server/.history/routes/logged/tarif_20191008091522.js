...
Tarifs promises
*/
async function getTarifsPromise(console_id) {
    await Tarif.find({console_id: console_id}, (err, tarifsData) => {
        if (err) {
            error.status = 500;
            next(error);
        }
...
router.get('/list', function (req, res, next) {
    ...
    try {
        Console.find({}).sort({ created_at: -1 }).exec(function (err, consolesData) {
            ...
            var consoles = [];
            var tarifs = [];
            const tarifsGetter = async () => {
                await globals.asyncForEach(consolesData, async (consoleData) => {
                    await Tarif.find({console_id: consoleData._id}, (err, tarifsData) => {
                        if (err) {
                            error.status = 500;
                            next(error);
                        }
            ...
            consolesData.forEach((consoleData, i) => {
